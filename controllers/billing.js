...
const addBilling = async (req, res) => {
    try {
        const {user_type, user_id  } = res.locals.auth.user;
       if(user_type === "admin"){
        const { invoice_no, invoice_date, delivery_note, mode_payment, other_reference, dis_throught, buyer_order_no, dispatched_no, dated,  delivery_note_date, designation, terms_of_del, productDetails, total_amount} = req.body;
